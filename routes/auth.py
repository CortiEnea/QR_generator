...
def login_post():
    ...
    user = QrUser.query.filter_by(email=email).first()
...
def signup_post():
    ...
    user = QrUser.query.filter_by(email=email).first() # if this returns a user, then the email already exists in database
    if user: 
    ...
    user = QrUser(username=username, email=email)
